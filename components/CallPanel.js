...
export default function CallPanel(){
  ...
  // RECHAMAR: cria nova entrada no historico E dispara anuncio (igual chamar novo)
  async function handleRecallLast(){
    if (!list.length) return;
    const last = list[0];
    const nome = String(last.nome || "").trim();
    const sala = String(last.sala || "").trim();
    if (!nome) return;
    ...
    try {
      ...
      await addDoc(collection(db, "calls"), {
        ...
        sala,
        timestamp: serverTimestamp(),
        recall: true, // marca como rechamada
      });
